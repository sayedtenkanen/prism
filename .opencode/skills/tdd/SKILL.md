---
name: tdd
description: Test-driven development with red-green-refactor loop. Use when user wants to build features or fix bugs using TDD, mentions "tdd", "test-driven", "red-green-refactor", "test first", "write tests", "test-first development", or asks for integration tests.
---
...
## Prism-Specific Checklist

Before committing after each TDD cycle, run:
...
- Unit tests: `tests/unit/` mirroring `app/` structure
...
When exploring the codebase, use Prism's domain glossary so that test names and interface vocabulary match the project's language, and respect existing patterns.
...
[ ] Run: ruff check . && ruff format --check . && mypy app/ && pytest tests/ -v --tb=short --cov=app --cov-report=term --cov-fail-under=90
```
